...
class TextDecoder(ModelMixin, ConfigMixin, ModuleUtilsMixin):
    ...
    def get_prefix_embeds(self, x, suffix_input_ids, add_bos_embeds=False):
        ...
        if add_bos_embeds:
            bos_token_ids = torch.full(size=(len(x), 1), fill_value=self.bos_token_id, dtype=torch.int64, device=x.device)
    ...
    def get_embedding_cat(self, input_ids, features, suffix_input_ids):
        assert (input_ids[:,0] == self.bos_token_id).all(), f"{self.bos_token_id}, {input_ids[:,0]}"
        embedding_text = self.transformer.transformer.wte(input_ids[:, 1:])
        prefix_embeds = self.get_prefix_embeds(features, suffix_input_ids, add_bos_embeds=True)
    ...
    def generate_captions(self, features, eos_token_id, device, suffix_input_ids=None):
        ...
        for feature in features:
            prefix_embeds = self.get_prefix_embeds(feature.to(device), suffix_input_ids, add_bos_embeds=True)
