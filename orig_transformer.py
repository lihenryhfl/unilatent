...
class SD3Transformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            ...
            if return_layers is not None and index_block in return_layers:
                ...
                if index_block == return_layers[-1]:
                    return_hidden = torch.cat(return_hidden, axis=-1)
                    return_hidden = (return_hidden, return_hidden)
                    return Transformer2DModelOutput(sample=torch.zeros_like(return_hidden[0]) / 0, hidden=return_hidden)
        ...
        if not return_dict:
            return (output, return_hidden)

        if return_hidden:
            return_hidden = torch.cat(return_hidden, axis=-1)
            return_hidden = (return_hidden, return_hidden)

        return Transformer2DModelOutput(sample=output, hidden=return_hidden)