...
def build_dataset(cfg, resolution=224, **kwargs):
    ...
    print(f"Constructing dataset {dataset_type}...")
    t = time.time()
    ...
    print(f"Dataset {dataset_type} constructed. time: {(time.time() - t):.2f} s, length (use/ori): {len(dataset)}/{dataset.ori_imgs_nums}")
    return dataset
