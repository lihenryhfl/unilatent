...
class UniLatentPipeline(DiffusionPipeline, FromSingleFileMixin):
    ...
    def decode_loss(self, image, prompt, device, dtype):
        ...
        if image_embd.shape[1] > prefix_length:
            if not hasattr(self, 'warn_truncate_image_embd'):
                print(f"Note: image_embd is shape {image_embd.shape} while prefix length is {prefix_length}. Truncating...")
                self.warn_truncate_image_embd = True
            image_embd = image_embd[:, :prefix_length]
        ...
        assert image_embd.shape[1] >= prefix_length, image_embd.shape
